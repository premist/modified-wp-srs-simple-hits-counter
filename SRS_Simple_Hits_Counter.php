<?php
/**
Plugin Name: SRS Simple hits Counter (Minku Lee Modified)
Plugin URI: https://github.com/premist/modified-wp-srs-simple-hits-counter
Description: Simple plugin to count and show a total number of hits (Unique visitors or page-views) to the site without using any third party code. (Modified)
Author: Atif N, Minku Lee
Version: 1.0.3
Author URI: https://github.com/premist
 */
...
function count_total_visitors_views($return, $date = null){
    global $wpdb;
    
    if ($date == 'today') {
        $date_formatted = Date('Y-m-d');
        $where = " WHERE srs_date = '$date_formatted'";
    ...
    $table_name = $wpdb->prefix . 'srs_simple_hits_counter';
    if ($return == 'views') {
        return $wpdb->get_results("SELECT SUM(srs_views_count) as total FROM $table_name $where")[0];
    } else {
        return $wpdb->get_results("SELECT SUM(srs_visitors_count) as total FROM $table_name $where")[0];
    }
}

// SHORTCODE for today
add_shortcode('srs_today_pageViews', 'srs_getToday_pageViews');
function srs_getToday_pageViews(){
    $data_return_views = count_total_visitors_views('views', 'today');
...
add_shortcode('srs_today_visitors', 'srs_getToday_visitors');
function srs_getToday_visitors(){
    $data_return_visitors = count_total_visitors_views('visitors', 'today');
    if(get_option('srs_pageViews_number_format_count') == 'yes'){
...
// SHORTCODE for yesterday
add_shortcode('srs_yesterday_pageViews', 'srs_getYesterday_pageViews');
function srs_getYesterday_pageViews(){
    $data_return_views = count_total_visitors_views('views', 'yesterday');
    if(get_option('srs_pageViews_number_format_count') == 'yes'){
        return "<span class='page-views'>".number_format(intval($data_return_views->total))."</span>";
    }else{
        return "<span class='page-views'>".intval($data_return_views->total)."</span>";
...
add_shortcode('srs_yesterday_visitors', 'srs_getYesterday_visitors');
function srs_getYesterday_visitors(){
    $data_return_visitors = count_total_visitors_views('visitors', 'yesterday');
    if(get_option('srs_pageViews_number_format_count') == 'yes'){
        return "<span class='visitors'>".number_format(intval($data_return_visitors->total))."</span>";
    }else{
        return "<span class='visitors'>".intval($data_return_visitors->total)."</span>";
